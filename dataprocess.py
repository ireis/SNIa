...
def select_spec_from_peak_by_bin (kpra_table, flux_matrix, w_matrix, tol, sn_names, min_wl, max_wl):
    ...
    return
    ------
    The indexes of required spectra ordered according to sn_names order
    """
    ...
    idx = [[] for x in range(len(sn_names))]
    for j in range (0,len(sn_names)):
        idx[j] = [i for i, e in zip(kpra_table['idx_kpr'].values, kpra_table['sn_kpr'].values) if e == sn_names[j]]
...
def create_umap_flux_mat (F_kpr, sn_names, idx, idx_min, idx_max):
    """
    create a matrix with spectra for good sne and nans for not in the order of sn_names and all the coloring we do later!
